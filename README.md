...
┌──────────────────────────────────────────────────────────────────────────────────┐
│                            👤 END USER (Browser)                                │
│  React 19 + Vite + TypeScript │ Framer Motion │ Socket.IO Client │ Google Fonts │
└────────────────────────────────────┬──────────────────┬──────────────────────────┘
...
│                                                                                  │
│  ┌─────────────────┐  ┌──────────────────┐  ┌────────────────────────────────┐  │
│  │  🛡️ MIDDLEWARE   │  │  🌐 API ROUTES    │  │  🔐 AUTH LAYER                │  │
│  │ • Request IDs   │  │ • /api/chat      │  │ • JWT dual-token (access +    │  │
│  │ • XSS Filter    │  │ • /api/battle    │  │   refresh)                    │  │
│  │ • Security Hdrs │  │ • /api/booths    │  │ • Google OAuth 2.0            │  │
│  │ • Audit Logging │  │ • /api/factcheck │  │ • Brute-force lockout         │  │
│  │ • Rate Limiting │  │ • /api/quiz      │  │ • OTP email verification      │  │
│  │ • CSP (Talisman)│  │ • /api/analytics │  │ • JWT blacklist (logout)      │  │
│  └─────────────────┘  │ • /api/database  │  │ • Fernet PII encryption       │  │
│                        └────────┬─────────┘  └────────────────────────────────┘  │
│                                 │                                                │
│  ┌──────────────────────────────▼───────────────────────────────────────────┐    │
│  │                    🧠 AGENTIC AI LAYER (5 Agents)                       │    │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────────┐  │    │
│  │  │Orchestr- │ │ Election │ │  Fact    │ │ Incident │ │   Debate     │  │    │
│  │  │  ator    │→│ Analyst  │ │ Checker  │ │Responder │ │  Moderator   │  │    │
│  │  │(Router)  │ │  📚      │ │  🔍     │ │  🚨     │ │    ⚔️       │  │    │
│  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────────┘  │    │
│  └──────────────────────────────┬───────────────────────────────────────────┘    │
│                                 │                                                │
...
│  │  ElectionClock → QueueDynamics (Poisson) → IncidentInjector             │    │
│  │  200 booths │ 10 constituencies │ ticks every 3s │ zero API calls       │    │
│  └──────────────────────────────────────────────────────────────────────────┘    │
└────────────┬──────────────┬──────────────┬──────────────┬────────────────────────┘
             │              │              │              │
             ▼              ▼              ▼              ▼
┌────────────────┐ ┌────────────────┐ ┌─────────────┐ ┌──────────────────────┐
│  🗄️ MySQL 8.0  │ │ 🔥 Firebase    │ │ 📦 Google   │ │  📊 Google Cloud     │
│  (Primary DB)  │ │ Firestore      │ │ Cloud       │ │  Logging             │
│ • Users        │ │ • Agent metrics│ │ Storage     │ │ • Auth events        │
│ • Sessions     │ │ • Leaderboard  │ │ • Debates   │ │ • Security alerts    │
│ • Booths       │ │ • Fact-checks  │ │ • Reports   │ │ • Agent actions      │
│ • Incidents    │ │ • Debates      │ │ • Snapshots │ │ • Incident lifecycle │
│ • Chat history │ │ • Analytics    │ │             │ │ • Audit trail        │
│ • Fact checks  │ │                │ │             │ │                      │
└────────────────┘ └────────────────┘ └─────────────┘ └──────────────────────┘
                            │                                     │
                            ▼                                     ▼
                   ...
                   │  Project: electaverse │ Zone: us-central1-a     │
                   │  ┌────────────────────────────────────────────┐  │
                   │  │  🖥️ Compute Engine (electaverse-server)    │  │
                   │  │  Docker containers │ Nginx reverse proxy   │  │
                   │  │  IP: 34.41.130.216                         │  │
                   │  └────────────────────────────────────────────┘  │
                   │  + Google Gemini API (AI) + Google OAuth 2.0     │
